...
export default function App() {
  ...
  const [caseResults, setCaseResults] = useState<
    Record<string, IntentionResult[]>
  >({});

  const [editingFlowId, setEditingFlowId] = useState<
    string | null
  >(null);
  const [showReportConfig, setShowReportConfig] =
    useState(false);
  const [selectedReportNode, setSelectedReportNode] =
    useState<FlowNode | null>(null);
  const [showTestRunModal, setShowTestRunModal] =
    useState(false);
  ...
  const handleAddNode = (type: string) => {
    ...
    if (
      type === "report" &&
      currentFlow.nodes.some((n) => n.type === "report")
    ) {
  ...
  const handleDeleteNode = (nodeId: string) => {
    if (!currentFlow) return;
    const newNodes = currentFlow.nodes.filter(
      (n) => n.id !== nodeId,
    );
    ...
    switch (currentView) {
      ...
      case "case":
              ...
              initialMessages={[
                {
                  id: "0",
                  sender: "user",
                  text: "I want to know more about Intention002",
                },
              ]}
            />
          );
        ...
        return (
          <CaseHistory
            cases={cases}
            onViewCase={(c) => setSelectedCase(c)}
          />
        );
      case "flow":
        return (
      ...
      default:
        return (
          <div className="p-8 ml-[320px] mt-[60px]">
            Select a view
          </div>
        );
    }
        ...
        <ReportConfiguration
          ...
          availableSources={
            currentFlow
              ? currentFlow.edges
                  .filter(
                    (e) => e.target === selectedReportNode.id,
                  )
                  .map((e) =>
